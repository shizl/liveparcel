function savenote(nid,atype){
	if(atype=="delete"){
		jQuery.post('/admin/liveparcels/settings/managepackage/ajax',{position:nid,atype:atype},function(data){
			if(data){
...
$(document).ready(function(){
	$.post("/admin/liveparcels/settings/managepackage/ajax",{atype:'packagelist'},function(data){
		if(data !='[]'){
			var ndata="";
	...
	$('#update-factor').click(function(){
		...
		if(factor>0){
            $.post('/admin/liveparcels/settings/updatefactor/ajax',{factor:factor},function(msg){
                if(msg){
    ...
    $('.zone-pickup').change(function(){
            ...
            $.post('/admin/liveparcels/settings/zoneresult/ajax',{region:region_id,zone:$(this).val(),oid:$('.oid').val(),oname:$('.oname').val(),ostate:$('.ostate').val(),path:$('.path_page').val()},function(msg){
                if(msg){
    ...
    $('.state-pickup').change(function(){
            $.post('/admin/liveparcels/settings/updatezone/ajax',{region:$(this).val()},function(msg){
                if(msg){
   ...
   $('.useregion').click(function(){
     ...
     $.post('/admin/liveparcels/settings/zoneresult/ajax',{region:'',zone:$('.zone-pickup').val(),oid:$('.oid').val(),oname:$('.oname').val(),ostate:$('.ostate').val(),path:$('.path_page').val()},function(msg){
            if(msg){
                $('.pickup-result .draggable').remove();
                $('.table-head').after(msg);
            }
