...
function copy_package(parcelnum){
 ...
 pickup_address = '';
	...
	pickup_address += '<option value="'+address_id[0]+'_'+parseInt(address_id[1]+1)+'_'+address_id[2]+'" >'+jQuery(this).text()+'</option>';
     });
 ...
 delivery_address = '';
     jQuery('fieldset #edit-delivery-'+parcelnum+'-select-address option').each(function(){
        address_id = jQuery(this).val().split('_');
	delivery_address += '<option value="'+address_id[0]+'_'+parseInt(address_id[1]+1)+'_'+address_id[2]+'" >'+jQuery(this).text()+'</option>';
     });
