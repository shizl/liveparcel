...
jQuery(".parcel_select_address").change(function(){
 ...
 jQuery('#content').prepend('<div class="messages error admin_order_error" style="display:none;"></div>');
jQuery('#admin-add-parcels-form .save').click(function(){
jQuery('.admin_order_error').css({'display':'block'});
...
time_setting = jQuery('.time_setting').val(); 
 ...
 size_width = jQuery('#edit-basic-info-'+i+' .size .form-item-width-'+i+' input').val();
 size_height = jQuery('#edit-basic-info-'+i+' .size .form-item-height-'+i+' input').val();
pweight = jQuery('#edit-basic-info-'+i+' .form-item-pweight-'+i+' input').val();
cash = jQuery('#edit-basic-info-'+i+' .form-item-cash-'+i+' input').val();
price = jQuery('#edit-parcel-price-'+i+' .form-item-price-'+i+' input').val();
 ...
 day = (mydate.getDate()+parseInt(time_setting))<10?'0'+(mydate.getDate()+parseInt(time_setting)):(mydate.getDate()+parseInt(time_setting));
   var fulldate = month +"/"+day+"/"+mydate.getFullYear();
    ...
    var arr2 = jQuery('.form-item-pickup-time-date-'+i+' input').val().split("/");
    var date2 = new Date(arr2[2], arr2[0], arr2[1]);

    if(isNaN(size_length)||parseFloat(size_length)<=0||size_length.length==0){
      jQuery('.admin_order_error').html('#parcel'+i+': length value must be Greater than 0');
      return false;
    }
    else if(isNaN(size_width)||parseFloat(size_width)<=0||size_width.length==0){
      jQuery('.admin_order_error').html('#parcel'+i+': width value must be Greater than 0');
      return false;
    }
    else if(isNaN(size_height)||parseFloat(size_height)<=0||size_height.length==0){
      jQuery('.admin_order_error').html('#parcel'+i+': height value must be Greater than 0');
      return false;
    }
    else if(isNaN(pweight)||parseFloat(pweight)<=0||pweight.length==0){
      jQuery('.admin_order_error').html('#parcel'+i+': weight value must be Greater than 0');
      return false;
    }
    else if(jQuery('.form-item-pickup-time-date-'+i+' input').val()==""){
      jQuery('.admin_order_error').html('#parcel'+i+': date can not be empty');
      return false;
    }
    else if(date1>date2){
      ...
      jQuery('.admin_order_error').html('#parcel'+i+': pickup first name can not be empty');
      return false;
    }
    else if(jQuery('.form-item-pickup-last-name-'+i+' input').val()==""){
      jQuery('.admin_order_error').html('#parcel'+i+': pickup last name can not be empty');
      return false;
    }
    else if(jQuery('.form-item-delivery-first-name-'+i+' input').val()==""){
      jQuery('.admin_order_error').html('#parcel'+i+': delivery first name can not be empty');
      return false;
    }
    else if(jQuery('.form-item-delivery-last-name-'+i+' input').val()==""){
      jQuery('.admin_order_error').html('#parcel'+i+': delivery last name can not be empty');
      return false;
    }
    else if(jQuery('.form-item-pickup-street1-'+i+' input').val()==""){
      jQuery('.admin_order_error').html('#parcel'+i+': pickup street can not be empty');
      return false;
    }
    else if(jQuery('.form-item-delivery-street1-'+i+' input').val()==""){
      jQuery('.admin_order_error').html('#parcel'+i+': delivery street can not be empty');
      return false;
    }
    else if(jQuery('.form-item-pickup-postal-code-'+i+' input').val()==""){
      jQuery('.admin_order_error').html('#parcel'+i+': pickup postal code can not be empty');
      return false;
    }
    else if(jQuery('.form-item-delivery-postal-code-'+i+' input').val()==""){
      jQuery('.admin_order_error').html('#parcel'+i+': delivery postal code can not be empty');
      return false;
    }
    else if(isNaN(price)||parseFloat(price)<=0||price.length==0){
      jQuery('.admin_order_error').html('#parcel'+i+': price value must be Greater than 0');
      return false;
    }
    else if(i==maxnum){
      return true;
                ...
                jQuery.get("/admin/liveparcels/getzoneprice/ajax",{ajaxrequest:"["+ajaxrequest+"]",action:"updateprice"},function(zone){
                   ...
                   if(zone != ""){ 
                         // $(".order ul").append(zone.parcels);
                          if( zone.error=="1"){
                              jQuery("#edit-parcel-price-"+parcelnum+" .display_pricre").html('<span style="color:red;">Warning: The calculator is unable to determine the cost to deliver the parcel to the location you have entered. </span>');
                            }else{	
                                  var pprice = 0;	
				  var packagename = '';		                               
...
function copy_package(parcelnum){
 ...
 am_select = pickup_ampm=='AM'?'selected="selected"':''; 
 pm_select = pickup_ampm=='PM'?'selected="selected"':''; 
...
jQuery('#parcel_warapper').find('#edit-parcels-'+maxnum).remove();
jQuery('.maxnum').val(maxnum-1);
...
function putpickuppost(parcelnum,type){

  zone = jQuery('.form-item-pickup-city-'+parcelnum+' input').val();
  zonename  = jQuery('.form-item-pickup-zone-'+parcelnum+' input').val();
  postal_code = jQuery('.form-item-pickup-postal-code-'+parcelnum+' input').val();

                jQuery.post("/liveparcels/lp_address_getzone/ajax",{zone:zone},function(data){
                    if(data){
                        jQuery('input[name="pickup_post_'+parcelnum+'"]').val(data.zone_number+'_'+zonename+'_'+postal_code);
                    }    
                },"json");

}

function putdeliverypost(parcelnum,type){

  zone = jQuery('.form-item-delivery-city-'+parcelnum+' input').val();
  zonename  = jQuery('.form-item-delivery-zone-'+parcelnum+' input').val();
  postal_code = jQuery('.form-item-delivery-postal-code-'+parcelnum+' input').val();
                jQuery.post("/liveparcels/lp_address_getzone/ajax",{zone:zone},function(data){
                    if(data){
                        jQuery('input[name="delivery_post_'+parcelnum+'"]').val(data.zone_number+'_'+zonename+'_'+postal_code);
                    }    
                },"json");
